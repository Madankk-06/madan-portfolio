...
const SplitFlapText = ({
  ...
  const sourceWords = Array.isArray(words) && words.length > 0 ? words : words;
  ...
  const [prevKey, setPrevKey] = useState(phrasesKey);
  const [tiles, setTiles] = useState(() => createTiles(normalizedPhrases[0] || ''));

  if (phrasesKey !== prevKey) {
    setPrevKey(phrasesKey);
    setTiles(createTiles(normalizedPhrases[0] || ''));
  }
  ...
  const dynamicFontSize = fontSize;
  ...
  const componentStyle = {
    ...
    '--split-flap-font-size': toCssUnit(dynamicFontSize),
